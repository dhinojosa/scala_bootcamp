...
class FunctionsSpec extends FunSuite with Matchers {
  ...
  test("""A function really is an anonymous
      |  instantiation of a trait.""".stripMargin) {

    pending
  }

  test("The above can be whittled down to x => f(x):") {
    pending
  }

  test("""If you declare the left hand side you
  ...
  test("""Lab: We will create a method called is,
  ...
  test("""Lab: A flatMap will take a function that returns
  ...
  test("""Lab: Find the average salary of employees of two departments
  ...
  test(
    """Lab: Using Map with a flatMap. In this lab, use flatMap or map to produce
      |the following:
      |
  ...
  test("""We will use a by name parameter to
  ...
  test("""Lab: Look up in the API a function in a collection that interests you,
