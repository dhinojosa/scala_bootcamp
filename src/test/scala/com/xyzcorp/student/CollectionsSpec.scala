...
class CollectionsSpec extends FunSuite with Matchers {
  test(
    """Case 1: We start our journey into collections with a List.
  ...
  test(
    """Case 3: To create a collection, there are varying signatures in
  ...
  test(
    """Case 4: For most purposes a Vector is preferable, fast random access,
  ...
  test(
    """Case 5: A range can be created with either to, until,
  ...
  test(
    """Case 6: If you have tried Scala, you have definitely encountered a Seq.
  ...
  test(
    """Case 7: A stream can be a finite or an infinite collection.
  ...
  test(
    """Case 9: An array is a java based array with a Scala wrapper
  ...
  test(
    """Case 10: Whereas a List is a LIFO, a Queue is a FIFO just like a
  ...
  test(
    """Case 11: A Set is a collection with unique items with no order. They can
  ...
  test(
    """Case 12: A Set is can also be a TreeSet.
  ...
  test(
    """Case 13: A Map is a collection of pairs, also known as Tuple2"""
      .stripMargin) {
  ...
  test(
    """Case 14: There are also mutable collections. Though much of the Scala
