...
class PatternMatchingSpec extends FunSpec with Matchers {
  ...
  describe("Simple Pattern Matches") {
    ...
    it(
      """Lab 1: Let's do up a replicate method using pattern matching using recursion"""
        .stripMargin) {
...
*/
    ...
    it(
      """Lab 2: Recreate the above but be sure that it is done
    ...
    it(
      """Lab 3: Implement mySecond, which will always return the
    ...
    it("""Can use a pipe or alternate matching to match an item""") {
      val any: Any = List(3)
      val str = any match {
    ...
    it(
      """Lab 4: Implement mySecond again, but this time use a pipe to clean
    ...
    it(
      """should have a None in a pattern match, though we have not covered it.
    ...
    it(
      """should also have guards just in case,
  ...
  describe("Custom pattern matching") {
    ...
    it("""while building a pattern match off of another unapply, in this
    ...
    it("""can also be used in composing partial functions to
  ...
  describe("""Companion Object Extractors for classes where the logic is in the
